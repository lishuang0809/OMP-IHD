function [A_hat, E_hat, iter] = exact_alm_rpca_SL(D, Q, lambda, tol, maxIter)
...
[U dummy dummy] = svdex(D - Q*temp_E + (1/mu)*Y, 1, struct('svd_solver','svds'));
while ~converged       
    iter = iter + 1;
    ...
    while primal_converged == false
        ...
        [U diagS V] = svdex(D - Q*temp_E + (1/mu)*Y, sv, struct('init',U(:,1)));
        % if choosvd(n, sv) == 1
