...
function[V_R,V_T,V_nsr] = Run_NewAlgs_4_Noise_updated(Q,S_opt,kappa,N_e,sigma_e,V_alg)
 ...
 for k=1:length(V_alg)
        ...
        [S,T,X_est] = NewAlgs_4_Noise_updated(Q,S_opt,X,kappa,W_bar,S_0,V_1,V_alg(k));
% The original S, Z                   
  [R,~] = ComputingRecoveryRate(S,S_opt,Q,rho_snr)  ; 
  V_R = [V_R R];           V_T = [V_T T];        % 1 x length(alg) 
  
% Computing relative MSE (i.e., NSR) 
  nsr = 0;
  for j=1:J
      e = X_opt(:,j) - X_est(:,j);
      nrs = nsr + (norm(e,2)/norm(X_opt(:,j),2))^2;
  end
  V_nsr = [V_nsr 10*log10(nrs/J+10^(-100))];
