...
function[S_kappa,T_c,X_est] = NewAlgs_4_Noise_imag_updated(Q,X,kappa,W_bar,S_0,V_1,alg)

[~,J] = size(X); L = size(Q,2);
S_kappa=zeros(L,J);    % estimated sparse vectors
...
% hold on;
%
% plot(kappa,V_4(:,1), '-v')   %  èçº¿
% hold on;
% plot(kappa,V_7(:,1), '-*')   % ç¹çº¿
% hold on;
% plot(kappa,V_8(:,1), '-x')   % ç¹åçº?
%
% axis([1 32  0 1])
%  xlabel('(a)')
% % ylabel('x(t),  x_1(t)');
...
% hold on;
%
% plot(kappa,V_4(:,2), '-v')   %  èçº¿
% hold on;
% plot(kappa,V_7(:,2), '-*')   % ç¹çº¿
% hold on;
% plot(kappa,V_8(:,2), '-x')   % ç¹åçº?
%
% axis([1 32  0 20])
%  xlabel('(b)')
% % ylabel('x(t),  x_1(t)');
