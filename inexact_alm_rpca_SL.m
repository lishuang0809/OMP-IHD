function [A_hat, E_hat, iter] = inexact_alm_rpca_SL(D, Q, lambda, tol, maxIter,mu)

% Modified by Shuang Li, Feb 2025.

% while ~converged 
%   minimize (inexactly, update A and E only once)
%     L(A,E,Y,u) = |A|_* + lambda * |E|_1 + <Y,D-A-Q*E> + mu/2 * |D-A-Q*E|_F^2;
%   Y = Y + \mu * (D - A - Q*E);
%   \mu = \rho * \mu;
% end
...
A_hat = zeros( m, n);
E_hat = zeros( L, n);
%mu = 1e3; %1.25/norm_two; % this one can be tuned
mu_bar = mu * 1e7;
...
% sv = 10;
% excelly
% temp_T = pinv(Q)*(D - A_hat + (1/mu)*Y);
% E_hat = sign(temp_T).*max(abs(temp_T) - lambda*mu, 0);
% %E_hat = E_hat+min(temp_T + lambda/mu, 0);
% [U, ~, ~] = svdex(D - Q*E_hat + (1/mu)*Y, 1, struct('svd_solver','svds'));
while ~converged       
    iter = iter + 1;
    
        %E_hat = max(temp_T - lambda/mu, 0);
    %E_hat = E_hat+min(temp_T + lambda/mu, 0);
    temp_TT = D - Q*E_hat - (1/mu)*Y;
    [U, SS, V] = svd(temp_TT);
    diagS = diag(SS);
    svp = length(find(diagS > mu));
    diagS = max(diagS - mu,0);
    A_hat = U(:, 1:svp) * diag(diagS(1:svp)) * V(:, 1:svp)';
    ...
    %[U, diagS, V] = svdex(D - Q*E_hat - (1/mu)*Y, sv, struct('init',U(:,1)));
    % if choosvd(n, sv) == 1
